...
function engagements_clear_engagement_object_cache( $engagementship_id ) {
	...
	wp_cache_delete( 'engagements_engagement_ids_' . $engagementship->receiver_user_id, 'bp' );
}
 ...
 */
function bp_engagements_clear_bp_engagements_engagementships_cache( $engagementship_id, $initiator_user_id, $receiver_user_id ) {
	// Clear engagementship ID cache for each user.
	wp_cache_delete( $initiator_user_id, 'bp_engagements_engagementships_for_user' );
	wp_cache_delete( $receiver_user_id, 'bp_engagements_engagementships_for_user' );
	...
	$engagementship->receiver_user_id    = $receiver_user_id;
...
function bp_engagements_clear_bp_engagements_engagementships_cache_remove( $engagementship_id, $engagementship ) {
	...
	wp_cache_delete( $engagementship->receiver_user_id, 'bp_engagements_engagementships_for_user' );
 ...
 * @since 2.0.0
 *
 * @param int $receiver_user_id The user ID not initiating the engagementship.
 */
function bp_engagements_clear_request_cache( $receiver_user_id ) {
	wp_cache_delete( $receiver_user_id, 'bp_engagements_requests' );
}
 ...
 * @param int $receiver_user_id    The user ID not initiating the engagementship.
 */
function bp_engagements_clear_request_cache_on_save( $engagementship_id, $initiator_user_id, $receiver_user_id ) {
	bp_engagements_clear_request_cache( $receiver_user_id );
}
...
function bp_engagements_clear_request_cache_on_remove( $engagementship_id, $engagementship ) {
	bp_engagements_clear_request_cache( $engagementship->receiver_user_id );
}
...
function bp_engagements_delete_cached_engagementships_on_engagementship_save( $engagementship ) {
	bp_core_delete_incremented_cache( $engagementship->receiver_user_id . ':' . $engagementship->initiator_user_id, 'bp_engagements' );
	bp_core_delete_incremented_cache( $engagementship->initiator_user_id . ':' . $engagementship->receiver_user_id, 'bp_engagements' );
}
