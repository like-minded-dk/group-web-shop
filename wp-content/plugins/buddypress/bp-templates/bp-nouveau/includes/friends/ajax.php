	...
	$ajax_actions = array(
		...
		array(
			'friends_not_engagements_from_friends' => array(
				'function' => 'bp_nouveau_ajax_addremove_friend_ext',
				'nopriv'   => false,
			),
		),
		array(
...
function bp_nouveau_ajax_addremove_friend_ext() {
	...
	// Trying to cancel engagementship in existed button.
		...
		ajax_remove_relation( 'engagement',  $engagement_id, $user_id, 'exist_more_engagements 197' );
	...
	// Trying to stop in existed button.
		...
		ajax_remove_relation( 'engagement',  $user_id, $engagement_id, '>>friends_remove_engagements_from_friends -f 495 ');

	// Trying to cancel pending request.
	} elseif ( 'not_engagement' === $check_is_engagement &&  $_POST['action'] == 'friends_remove_engagements_from_friends') {
		// todo : user ajax_remove_engagement 'contents' => bp_get_add_friend_button( $friend_id ) );
		ajax_remove_relation( 'engagement',  $user_id, $engagement_id, '>>not_engagement -f 435 ');
	...
	// Trying to cancel pending request
		...
		ajax_add_relation($user_id, $engagement_id, '>>exist_initiator_engagement -f 434: '. $user_id . ' - ' . $engagement_id );
