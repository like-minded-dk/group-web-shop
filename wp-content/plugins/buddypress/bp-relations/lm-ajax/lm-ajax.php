...
$ajax_function = new LM_Ajax_Function();
...
function bp_nouveau_ajax_addremove_fn_friend() {
	$ajax_function = new LM_Ajax_Function('friend');
	$ajax_function->lm_ajax_run_addremove_fn('friend');
}

function bp_nouveau_ajax_addremove_fn_engagement() {
	$ajax_function = new LM_Ajax_Function('engagement');
	$ajax_function->lm_ajax_run_addremove_fn('engagement');
}
...
class LM_Ajax_Function {
	// private function __construct() {
	// 	/* Do nothing here */
	// }

	public function lm_ajax_run_addremove_fn($comp) {
		$response = array(
			'feedback' => sprintf(
		...
		self::check_nonce($comp, $response);
		// Cast fid as an integer.
		self::check_user_exist($comp, $item_id);
		self::switch_each_action($comp, $action, $user_id, $item_id, $response);
	}
	
	private function add_relation($comp, $user_id, $item_id, $response, $error = '', $note='') {
	...
	private function withdraw_relation($comp, $user_id, $item_id, $response,  $error = '', $note='') {
	...
	private function remove_relation($comp, $user_id, $item_id, $response, $error='', $note ='') {
	...
	private function reject_relation($comp, $item_id, $response, $error = '') {
	...
	private function accept_relation($comp, $item_id, $response, $error = '') {
	...
	private function switch_each_action($comp, $action, $user_id, $item_id, $response) {
		...
		// Trying to add relationship.
			...
			self::add_relation( $comp,  $user_id, $item_id, $response, ' [action] add: ' .  $comp . ' ' . $user_id . ' - ' . $item_id);
		
		// Trying to withdraw pending relationship.
		} elseif ( $action === $withdraw_action) {
			self::withdraw_relation($comp, $user_id, $item_id, $response, ' [action] withdraw: ' . $comp . ' ' . $user_id . ' - ' . $item_id);
	
		// Trying to remove relationship.
		} elseif ( $action === $remove_action ) {
			self::remove_relation( $comp,  $user_id, $item_id, $response, ' [action] remove: ' . $comp . ' ' . $user_id . ' - ' . $item_id);	
	
		// Trying to remove relationship reverse.
		} elseif ( $action === $remove_action_reversed ) {
			self::remove_relation( $oppo,  $item_id, $user_id, $response, ' [action] remove_reverse: ' . $comp . ' ' . $user_id . ' - ' . $item_id);	
	...
	private function check_nonce($comp, $error_response) {
	...
	private function check_user_exist($comp, $item_id) {
		if ($comp == 'friend') {
			$accpt_action = 'friends_accept_friend';
			$reject_action = 'friends_reject_friend';
		} else {
