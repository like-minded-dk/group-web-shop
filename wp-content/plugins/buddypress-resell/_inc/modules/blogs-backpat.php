 ...
 * 'resellblogs' activity scope for those using BP 2.0 - 2.1.
 *
 * @since 1.3.0
 *
 * @package BP-Resell
...
/**
 * Set activity scope on a user's "Activity > Reselled Sites" page
 */
function bp_resell_blogs_set_activity_scope_on_user_activity() {
	if ( ! bp_is_current_action( constant( 'BP_RESELL_BLOGS_USER_ACTIVITY_SLUG' ) ) ) {
		return;
	}
...
add_action( 'bp_before_activity_loop', 'bp_resell_blogs_set_activity_scope_on_user_activity' );

/**
 * Filter the activity loop.
 *
 * Specifically, when on the activity directory and clicking on the "Reselled
 * Sites" tab.
 *
 ...
 */
function bp_resell_blogs_add_activity_scope_filter( $qs, $object ) {
	...
	if ( function_exists( 'bp_groupblog_init' ) ) {
		add_filter( 'bp_activity_get_where_conditions', 'bp_resell_blogs_groupblog_activity_where_conditions', 10, 2 );
	}

	return $qs;
}
add_filter( 'bp_ajax_querystring', 'bp_resell_blogs_add_activity_scope_filter', 20, 2 );
 ...
 * @return array
 */
function bp_resell_blogs_groupblog_activity_where_conditions( $retval, $r ) {
	$bp = $GLOBALS['bp'];
	...
	remove_filter( 'bp_activity_get_where_conditions', 'bp_resell_blogs_groupblog_activity_where_conditions', 10, 2 );
	return $retval;
}
